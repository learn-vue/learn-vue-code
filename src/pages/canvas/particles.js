...
function Particles(c){
	...
	this.init=(function(){
		...
		if(!c.imgUrl){console.log("pls use the correct img url");return}
		...
		this.draw=function(){
			if(image.isLoaded){Particles.prototype._draw()}else{setTimeout(a.draw)}};
			this.animate=function(){
			if(image.isLoaded){Particles.prototype._animate(c.delay)}else{setTimeout(a.animate)}}}Particles.prototype={array:[],_calculate:function(a){var h=image.imgData.length;
			...
			d++){for(var b=0;
				b<n;
			b++){k=(b*c*image.w+d*m)*4;
				if(f[k]<=10){this.array.push({x0:a.startX,y0:a.startY,x1:image.x+d*m+(Math.random()-0.5)*10*a.pOffset,y1:image.y+b*c+(Math.random()-0.5)*10*a.pOffset,fillStyle:a.color,delay:b/20,currTime:0,count:0,duration:parseInt(a.duration/16.66)+1,interval:parseInt(Math.random()*10*a.interval),ease:a.ease,ratioX:a.ratioX,ratioY:a.ratioY})}}}},_draw:function(){
			canvas.ctx.clearRect(0,0,canvas.w,canvas.h);
		...
		canvas.ctx.fillRect(a.x1,a.y1,1,1)}},_render:function(){
			canvas.ctx.clearRect(0,0,canvas.w,canvas.h);
			...
			for(var e=0;
				e<f;
			e++){h=l[e];
				if(h.count++>h.delay){canvas.ctx.fillStyle=h.fillStyle;
				k=h.currTime;
			b=h.duration;
			c=h.interval;
			h.ratioX!==1?j=h.ratioX+Math.random()*2:1;
			h.ratioY!==1?g=h.ratioY+Math.random()*2:1;
			if(l[f-1].duration+l[f-1].interval<l[f-1].currTime/2){cancelAnimationFrame(requestId);
				Particles.prototype._draw();
			return}else{if(k<b+c){if(k>=c){
		d=_ease[h.ease]((k-c)*j,h.x0,(h.x1-h.x0)*j,b);
		a=_ease[h.ease]((k-c)*g,h.y0,(h.y1-h.y0)*g,b);
		canvas.ctx.fillRect(d,a,1,1)}}else{canvas.ctx.fillRect(h.x1,h.y1,1,1)}}h.currTime+=Math.random()+0.5}}requestId=requestAnimationFrame(Particles.prototype._render)},_animate:function(a){if(startTime+a<new Date().getTime()){requestId=requestAnimationFrame(Particles.prototype._render)}else{setTimeout(function(){
			Particles.prototype._animate(a)})}}};

		var _ease = {
			linear: function(e,a,g,f){return g*e/f+a},
